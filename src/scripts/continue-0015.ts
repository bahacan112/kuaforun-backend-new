...
async function continueFrom0015() {
  try {
    console.log('Continuing manual migration from 0015...');
    
    // Read all migration files starting from 0015 (skip the problematic 0014)
    const migrationsDir = path.join(process.cwd(), 'drizzle');
    const files = fs.readdirSync(migrationsDir)
      .filter(file => file.endsWith('.sql'))
      .sort()
      .filter(file => file > '0014_allow_overlap_on_cancelled.sql'); // Skip 0014 and continue from 0015
