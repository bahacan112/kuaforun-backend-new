...
async function continueMigration() {
  try {
    console.log('Continuing manual migration process...');
    
    // Read all migration files starting from 0005
    const migrationsDir = path.join(process.cwd(), 'drizzle');
    const files = fs.readdirSync(migrationsDir)
      .filter(file => file.endsWith('.sql'))
      .sort()
      .filter(file => file >= '0005_shop_owner_replies.sql'); // Continue from 0005
