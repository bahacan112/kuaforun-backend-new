...
async function continueFrom0010() {
  try {
    console.log('Continuing manual migration from 0010...');
    
    // Read all migration files starting from 0010
    const migrationsDir = path.join(process.cwd(), 'drizzle');
    const files = fs.readdirSync(migrationsDir)
      .filter(file => file.endsWith('.sql'))
      .sort()
      .filter(file => file >= '0010_abandoned_wolf_cub.sql'); // Continue from 0010
