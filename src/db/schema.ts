...
export const users = pgTable(
  "users",
  {
    ...
    registrationApprovedAt: timestamp("registration_approved_at", {
      withTimezone: true,
    }),
...
export const barberShops = pgTable(
  ...
  (table) => ({
    ...
    tenantIdx: index("barber_shops_tenant_idx").on(table.tenantId),
  })
);
...
export const barberComments = pgTable(
  "barber_comments",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    barberShopId: uuid("barber_shop_id")
  ...
  (table) => ({
    uniquePerBooking: uniqueIndex("barber_comment_unique_booking").on(
      table.userId,
      table.bookingId
    ),
  })
...
export const shopStaff = pgTable(
  ...
  (table) => ({
    // Aynı dükkanda aynı kullanıcı sadece bir kez çalışabilir
    shopUserUnique: uniqueIndex("shop_staff_shop_user_unique").on(
      table.shopId,
      table.userId
    ),
  })
